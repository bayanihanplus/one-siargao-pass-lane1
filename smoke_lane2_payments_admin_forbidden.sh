...
expect_http 403 "1) TRAVELER CANNOT READ ADMIN PAYMENT INTENT" \
  curl "$BASE/payments/admin/intents/$KNOWN_INTENT_ID" \
  -H "Authorization: Bearer $TRAVELER_TOKEN"

expect_http 403 "2) TRAVELER CANNOT READ ADMIN BOOKING PAYMENT STATE" \
  curl "$BASE/payments/admin/states/$KNOWN_BOOKING_ID" \
  -H "Authorization: Bearer $TRAVELER_TOKEN"

expect_http 403 "3) OPERATOR CANNOT READ ADMIN PAYMENT INTENT" \
  curl "$BASE/payments/admin/intents/$KNOWN_INTENT_ID" \
  -H "Authorization: Bearer $OPERATOR_TOKEN"

expect_http 403 "4) OPERATOR CANNOT READ ADMIN BOOKING PAYMENT STATE" \
  curl "$BASE/payments/admin/states/$KNOWN_BOOKING_ID" \
  -H "Authorization: Bearer $OPERATOR_TOKEN"

echo "=== LANE 2 PAYMENT ADMIN FORBIDDEN SMOKE PASSED ==="
