...
function getPassReadinessSummary(trip: any, passUsageWarnings: string[], dictionary: TravelerDictionary) {
  ...
  if (trip?.pass?.qrCredential?.qrToken) {
    return {
      ...
      body: t(
        dictionary,
        "pass.readiness.ready.body",
        "Your OSP Pass and QR credential are active in your traveler record. Some activities may still require separate clearance, payment, or manifest validation.",
      ),
      accent: "#16a34a",
    };
  }
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                {passUsageWarnings.length > 0 ? (
                  <p style={{ marginTop: 12, marginBottom: 0, color: "#b45309", fontWeight: 600 }}>
                    {t(dictionary, "pass.qr.notReady", "Your OSP QR credential is active in your traveler record.")}
                  </p>
                ) : null}
              </div>
            ) : (
