...
const passDictionaryFallback: TravelerDictionary = {
  ...
  "pass.readiness.ready.body": "Your OSP Pass and QR credential are active in your traveler record. Some activities may still require separate clearance, payment, or manifest validation.",
  "pass.readiness.onRecord.title": "Pass Active",
  ...
  "pass.qr.notReady": "Your OSP QR credential is active in your traveler record.",
...
function getPassReadinessSummary(trip: any, passUsageWarnings: string[], dictionary: TravelerDictionary) {
  ...
  if (trip?.pass?.qrCredential?.qrToken) {
    return {
      tone: "green",
      icon: "✓",
      title: t(dictionary, "pass.readiness.ready.title", "Pass Ready"),
      body: "Your OSP Pass and QR credential are active in your traveler record.",
      accent: "#16a34a",
    };
  }
  ...
  if (trip?.pass) {
    return {
      tone: "gold",
      icon: "!",
      title: t(dictionary, "pass.readiness.onRecord.title", "Pass Active"),
      body: "Your pass is active. QR credential display depends on the pass record.",
      accent: "#d97706",
    };
  }
...
export default async function TravelerPassPage() {
  ...
  return (
    ...
    >
          ...
          <Section title={t(dictionary, "pass.readiness.sectionTitle", "Pass Readiness")}>
            ...
            >
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                ...
                <PassportMapShortcut compact title="Open Passport Map" body="Use your OSP Pass with the Passport Map to continue your verified Siargao journey." />
            ...
            {trip.pass?.qrCredential?.qrToken ? (
              <div style={{ marginBottom: 16 }}>
                ...
                {passUsageWarnings.length > 0 ? (
                  <p style={{ marginTop: 12, marginBottom: 0, color: "#0f766e", fontWeight: 600 }}>
                    {"Your OSP QR credential is active in your traveler record."}
                  </p>
                ) : null}
              </div>
            ) : (
      ...
      <UniversalTravelerBottomTabBar fixed />
    </main>
  );
}
