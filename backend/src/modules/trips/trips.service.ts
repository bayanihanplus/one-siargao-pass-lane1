...
export class TripsService {
  ...
  async getTravelerHomeSummary(userId: string | undefined) {
    if (!userId) {
      return {
        ok: false,
        ...
        passLayer: {
          layer: 'TRAVELER_TRIP_PARTICIPATION',
          hasTrip: false,
          hasPass: false,
          hasQrCredential: false,
          passCode: null,
          passStatus: null,
          validFrom: null,
          validUntil: null,
        ...
        clearanceLayer: {
          layer: 'REGULATED_ACTIVITY_PERMISSION',
          requiresClearance: false,
          clearanceScope: 'NONE',
          regulatedActivityType: null,
          clearanceStatus: null,
          travelerFacingStatus: 'NOT_REQUIRED',
        },
        serverDecisionFlags: {
          hasAuthenticatedUser: false,
          hasTrip: false,
          hasPass: false,
          hasQrCredential: false,
          isTripDateWindowValid: false,
          isPassIssued: false,
          isClearanceApproved: false,
          isPaymentPaid: false,
          isTravelerHomeReady: false,
        },
    ...
    // Backend truth safety net:
    // If an authenticated traveler has a trip but no OSP pass row yet,
    // create/ensure the canonical pass + QR spine from the backend.
    if (latestTrip && !pass) {
      const ensuredPass = await ensureOspPassForTrip(this.prisma as any, {
        tripId: latestTrip.id,
        travelerUserId: userId,
        expiresAt: latestTrip.departureDate
          ? new Date(new Date(latestTrip.departureDate).getTime() + 1000 * 60 * 60 * 24)
          : undefined,
        source: 'api_trip_create',
        metadataJson: {
      ...
      pass = ensuredPass || null;
    }

    const qrCredential = pass?.qrCredential || null;
    const primaryBookingLink = latestTrip?.bookingLinks?.[0] || null;
    const booking = primaryBookingLink?.booking || null;
    ...
    const isArrivalValid = Boolean(arrivalDate && !Number.isNaN(arrivalDate.getTime()));
    const isDepartureValid = Boolean(departureDate && !Number.isNaN(departureDate.getTime()));
    const isTripDateWindowValid =
      Boolean(
        isArrivalValid &&
          isDepartureValid &&
          arrivalDate !== null &&
    ...
    const isCurrentlyWithinTripWindow =
      Boolean(
        isArrivalValid &&
          isDepartureValid &&
          arrivalDate !== null &&
    ...
    const passStatus = String(pass?.passStatus || '').toUpperCase();
    const clearanceStatus = String(latestTrip?.clearanceStatus || '').toUpperCase();
    const paymentStateValue = String(paymentState?.state || latestPaymentIntent?.status || '').toUpperCase();

    const regulatedSourceText = [
      latestTrip?.tripTitle,
      latestTrip?.originLocation,
      latestTrip?.declaredAccommodationName,
      (booking as any)?.bookingReference,
      (booking as any)?.bookingType,
      (booking as any)?.serviceType,
      (booking as any)?.serviceCategory,
      (booking as any)?.metadataJson ? JSON.stringify((booking as any).metadataJson) : null,
    ]
      .filter(Boolean)
      .join(' ')
      .toLowerCase();

    const isIslandHoppingTrip =
      regulatedSourceText.includes('island hopping') ||
      regulatedSourceText.includes('island-hopping') ||
      regulatedSourceText.includes('tri island') ||
      regulatedSourceText.includes('tri-island') ||
      regulatedSourceText.includes('naked island') ||
      regulatedSourceText.includes('daku island') ||
      regulatedSourceText.includes('guyam island') ||
      regulatedSourceText.includes('sohoton') ||
      regulatedSourceText.includes('bucas grande');

    const requiresClearance = Boolean(isIslandHoppingTrip);

    const clearanceLayer = {
      layer: 'REGULATED_ACTIVITY_PERMISSION',
      requiresClearance,
      clearanceScope: requiresClearance ? 'ISLAND_HOPPING' : 'NONE',
      regulatedActivityType: requiresClearance ? 'ISLAND_HOPPING' : null,
      clearanceStatus: requiresClearance ? latestTrip?.clearanceStatus || null : null,
      travelerFacingStatus: requiresClearance
        ? latestTrip?.clearanceStatus || 'PENDING'
        : 'NOT_REQUIRED',
    };
    ...
    const identityLayer = {
      ...
      travelerUserId: userId,
      hasQrIdentity: Boolean(qrCredential),
      qrToken: qrCredential?.qrToken || null,
      note: 'QR identity belongs to the traveler/person layer and must not wait for regulated activity clearance.',
    };
    ...
    const passLayer = {
      ...
      hasQrCredential: Boolean(qrCredential),
      passCode: pass?.passCode || null,
      passStatus: pass?.passStatus || null,
      validFrom: latestTrip?.arrivalDate || null,
      validUntil: pass?.expiresAt || latestTrip?.departureDate || null,
      note: 'OSP Pass is tied to traveler/trip participation and valid dates, not universal LGU clearance.',
    };

    const isPassIssued = Boolean(pass && ['ACTIVE', 'ISSUED', 'VALID'].includes(passStatus));
    const isClearanceApproved = clearanceStatus === 'APPROVED';
    const isPaymentPaid = paymentStateValue === 'PAID';
    ...
    return {
      ...
      latestTrip: latestTrip
        ? {
            id: latestTrip.id,
            tripTitle: latestTrip.tripTitle,
            arrivalDate: latestTrip.arrivalDate,
            departureDate: latestTrip.departureDate,
            originLocation: latestTrip.originLocation,
            declaredAccommodationName: latestTrip.declaredAccommodationName,
            tripStatus: latestTrip.tripStatus,
            registrationStatus: latestTrip.registrationStatus,
            clearanceStatus: latestTrip.clearanceStatus,
            createdAt: latestTrip.createdAt,
            updatedAt: latestTrip.updatedAt,
          }
        : null,
      tripValidity: latestTrip
        ? {
            source: 'Trip.arrivalDate + Trip.departureDate',
            arrivalDate: latestTrip.arrivalDate,
            departureDate: latestTrip.departureDate,
            isDateWindowValid: isTripDateWindowValid,
            isCurrentlyWithinWindow: isCurrentlyWithinTripWindow,
          }
        : null,
      passSummary: pass
        ? {
        ...
        : null,
      qrSummary: qrCredential
        ? {
            id: qrCredential.id,
            qrToken: qrCredential.qrToken,
            qrVersion: qrCredential.qrVersion,
            lastRegeneratedAt: qrCredential.lastRegeneratedAt,
      ...
      paymentSummary: {
        ...
        paymentState: paymentState
          ? {
              ...
              paidAmountPhp: paymentState.paidAmountPhp,
              unpaidAmountPhp: paymentState.unpaidAmountPhp,
              lastPaymentIntentId: paymentState.lastPaymentIntentId,
              stateUpdatedAt: paymentState.stateUpdatedAt,
            }
          : null,
        latestPaymentIntent: latestPaymentIntent
          ? {
              id: latestPaymentIntent.id,
              intentReference: latestPaymentIntent.intentReference,
              amountPhp: latestPaymentIntent.amountPhp,
              currencyCode: latestPaymentIntent.currencyCode,
              status: latestPaymentIntent.status,
              provider: latestPaymentIntent.provider,
              confirmedAt: latestPaymentIntent.confirmedAt,
              createdAt: latestPaymentIntent.createdAt,
            }
          : null,
      },
      clearanceSummary: {
        current: latestTrip?.clearanceStatus || null,
        latestHistory: latestTrip?.clearanceHistory || [],
      },
      manifestSummary: {
        isManifestListed: Boolean(latestTrip?.manifestMembers?.length),
        manifestMemberCount: latestTrip?.manifestMembers?.length || 0,
      },
      identityLayer,
      passLayer,
      clearanceLayer,
      serverDecisionFlags: {
        ...
        hasPass: Boolean(pass),
        hasQrCredential: Boolean(qrCredential),
        isTripDateWindowValid,
        isPassIssued,
        isClearanceApproved: requiresClearance ? isClearanceApproved : true,
        isPaymentPaid,
        isTravelerHomeReady: Boolean(latestTrip && isPassIssued && (requiresClearance ? isClearanceApproved : true)),
      },
      generatedAt: new Date().toISOString(),
    };
  }
