...
export default function CultureCommunityConfirmPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const nextHref = buildHref("/traveler/passport-trails/culture-community/route-confirmed", nextParams);
  const backHref = buildHref("/traveler/passport-trails/culture-community/book", nextParams);
  ...
  return (
    <main style={{ minHeight: "100vh", background: "linear-gradient(180deg, #F4FCFA 0%, #FFFFFF 45%, #EAFBFA 100%)", color: OSP.deepNavy, padding: "16px 14px 118px", fontFamily: '"Source Sans 3", "Source Sans Pro", "Noto Sans", Arial, sans-serif' }}>
      <section style={{ width: "100%", maxWidth: 430, margin: "0 auto" }}>
        <Link href={backHref} style={{ color: OSP.teal, textDecoration: "none", fontSize: 13, fontWeight: 850 }}>← Edit community session</Link>
        ...
        <header style={{ marginTop: 14, background: OSP.white, border: `1px solid ${OSP.line}`, borderRadius: 28, boxShadow: "0 18px 50px rgba(1,56,99,0.08)", padding: 20 }}>
          <div style={{ color: OSP.teal, fontSize: 10.5, fontWeight: 900, letterSpacing: "0.12em", textTransform: "uppercase" }}>Culture & Community Trail</div>
          <h1 style={{ margin: "5px 0 0", fontSize: 25, lineHeight: 1.02, fontWeight: 760, letterSpacing: "-0.045em" }}>Review community session</h1>
          <p style={{ margin: "10px 0 0", color: OSP.slate, fontSize: 13.2, lineHeight: 1.42, fontWeight: 720 }}>
            Community partner support is confirmed into this session context before checkout.
          </p>
        </header>
        ...
        <section style={{ marginTop: 14, background: OSP.white, border: `1px solid ${OSP.line}`, borderRadius: 28, boxShadow: "0 18px 50px rgba(1,56,99,0.08)", padding: 16 }}>
          {[
            ...
            ["Trip", tripNo],
            ["Route", "Community partner session"],
