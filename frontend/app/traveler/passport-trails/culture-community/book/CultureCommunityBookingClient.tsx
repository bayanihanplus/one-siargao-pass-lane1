...
type CultureSessionCode =
  | "CULTURE_COMMUNITY_SESSION"
  | "LOCAL_STORY_WALK"
  | "COMMUNITY_PARTNER_VISIT";

type MeetingZoneCode =
  | "GENERAL_LUNA_COMMUNITY_MEETUP"
  | "GL_POBLACION"
  | "CATANGNAN_COMMUNITY_POINT"
...
const CULTURE_SESSIONS: Record<CultureSessionCode, { label: string; routeCode: string; note: string }> = {
  CULTURE_COMMUNITY_SESSION: {
    label: "Community Culture Session",
    routeCode: "culture-community-partner-session",
    note: "Partner-supported cultural encounter",
  },
  LOCAL_STORY_WALK: {
    label: "Local Story Walk",
    routeCode: "culture-community-local-story-walk",
    note: "Local story and cultural stop context",
  },
  COMMUNITY_PARTNER_VISIT: {
    label: "Community Partner Visit",
    routeCode: "culture-community-partner-visit",
    note: "Community partner stop and visitor flow",
  },
};

const MEETING_ZONES: Record<MeetingZoneCode, { label: string; note: string }> = {
  GENERAL_LUNA_COMMUNITY_MEETUP: {
    label: "General Luna community meetup",
    note: "Primary community meeting context",
  },
  GL_POBLACION: {
  ...
  CATANGNAN_COMMUNITY_POINT: {
    label: "Catangnan community point",
    note: "Community point review may apply",
  },
  REQUEST_MEETING_REVIEW: {
...
export default function CultureCommunityBookingClient() {
  const [session, setSession] = useState<CultureSessionCode>("CULTURE_COMMUNITY_SESSION");
  const [meetingZone, setMeetingZone] = useState<MeetingZoneCode>("GENERAL_LUNA_COMMUNITY_MEETUP");
  const [date, setDate] = useState(todayIso());
  const [departureTime, setDepartureTime] = useState("09:00");
  const [regularPax, setRegularPax] = useState(1);
  const [seniorPax, setSeniorPax] = useState(0);

  const product = CULTURE_SESSIONS[session];
  ...
  const tripNo = `SPM-CULTURE-${date.replaceAll("-", "")}-${departureTime.replace(":", "")}`;

  const confirmHref = useMemo(() => {
    return buildHref("/traveler/passport-trails/culture-community/confirm", {
      ...
      pricingVersion: "CULTURE_COMMUNITY_2026_05",
      pricingMode: "CULTURE_COMMUNITY_PER_PAX",
      paymentTiming: "AFTER_PARTNER_CONFIRMATION",
      currencyCode: "PHP",
      step: "confirm",
    });
    ...
    <main style={{ minHeight: "100vh", background: "linear-gradient(180deg, #F4FCFA 0%, #FFFFFF 45%, #EAFBFA 100%)", color: OSP.deepNavy, padding: "16px 14px 118px", fontFamily: '"Source Sans 3", "Source Sans Pro", "Noto Sans", Arial, sans-serif' }}>
      <section style={{ width: "100%", maxWidth: 430, margin: "0 auto" }}>
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          ...
          <Link href="/traveler/passport-trails/culture-community" style={{ borderRadius: 999, border: `1px solid ${OSP.line}`, background: OSP.white, color: OSP.teal, padding: "10px 13px", fontSize: 11.5, fontWeight: 850, whiteSpace: "nowrap", textDecoration: "none" }}>
            Trail
          </Link>
        </header>
        ...
        <section style={{ marginTop: 14, borderRadius: 28, background: OSP.white, border: `1px solid ${OSP.line}`, boxShadow: "0 18px 50px rgba(1,56,99,0.08)", padding: 14 }}>
          ...
          <div style={{ display: "grid", gap: 9 }}>
            {(Object.entries(CULTURE_SESSIONS) as Array<[CultureSessionCode, (typeof CULTURE_SESSIONS)[CultureSessionCode]]>).map(([code, item]) => {
        ...
        <section style={{ marginTop: 12, borderRadius: 24, background: OSP.white, border: `1px solid ${OSP.line}`, padding: 12 }}>
          <p style={{ margin: 0, color: OSP.slate, fontSize: 12.2, lineHeight: 1.35, fontWeight: 720 }}>
            This flow confirms community session context before payment. It does not create a DCS boarding, manifest, or port movement record.
          </p>
        </section>
