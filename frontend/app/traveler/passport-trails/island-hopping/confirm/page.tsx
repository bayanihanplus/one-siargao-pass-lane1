...
export default function IslandHoppingConfirmPage({ searchParams }: { searchParams: SearchParams }) {
  const flowParams = buildFlowParams(searchParams, "island-hopping-seats-confirmed", "seats-confirmed");

  return (
    <main style={shell}>
      <section style={{ width: "100%", maxWidth: 390, margin: "0 auto", display: "grid", gap: 14 }}>
        <section style={card}>
          <span style={eyebrow}>Route review</span>
          <h1 style={{ margin: "10px 0 6px", fontSize: 25, lineHeight: 1.02, letterSpacing: "-0.04em" }}>Confirm your seat.</h1>
          <p style={{ margin: 0, color: "#50668B", fontSize: 12.2, lineHeight: 1.28, fontWeight: 750 }}>Review pax, route, and amount before payment.</p>
        </section>

        <section style={{ ...card, display: "grid", gap: 10 }}>
          <SummaryRow label="Route" value="GL Tri-Island" />
          <SummaryRow label="Product" value={productLabel(flowParams.product)} />
          <SummaryRow label="Date" value={flowParams.date} />
          <SummaryRow label="Time" value={flowParams.departureWindow} />
          <SummaryRow label="Travelers" value={`${flowParams.pax} pax`} />
          <SummaryRow label="Boat class" value={flowParams.boatClass} />
          <SummaryRow label="Amount" value={formatPeso(flowParams.amount)} highlight />
        </section>

        <Link href={makeHref("/traveler/passport-trails/island-hopping/seats-confirmed", flowParams)} style={{ display: "grid", placeItems: "center", minHeight: 56, borderRadius: 18, background: "#013863", color: "#FFFFFF", textDecoration: "none", fontSize: 14, fontWeight: 950 }}>
          Confirm Seats & Continue
        </Link>

        <Link href={makeHref("/traveler/passport-trails/island-hopping/book", flowParams)} style={{ display: "grid", placeItems: "center", minHeight: 52, borderRadius: 18, background: "#FFFFFF", color: "#013863", textDecoration: "none", fontSize: 13, fontWeight: 950, border: "1px solid rgba(5,150,165,0.14)" }}>
          Back to Seat Setup
        </Link>
      </section>
